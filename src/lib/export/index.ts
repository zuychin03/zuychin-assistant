...
export async function renderDocument(
    ...
    format: ExportFormat,
    title: string,
): Promise<{ body: Buffer | string; mimeType: string; filename: string }> {
    ...
    if (format === "docx") {
        return { body: await generateDocx(content, safeTitle), mimeType, filename };
    }
    return { body: await generatePdf(content, safeTitle), mimeType, filename };
}
